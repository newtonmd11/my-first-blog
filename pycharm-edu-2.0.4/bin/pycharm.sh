...
#
# ---------------------------------------------------------------------
# PyCharm Edu startup script.
# ---------------------------------------------------------------------
#

message()
{
  TITLE="Cannot start PyCharm Edu"
...
user_vm_options_file="$HOME/.PyCharmEdu20/pycharm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=PyCharmEdu20"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Djb.restart.code=88 -Didea.paths.selector=PyCharmEdu20 \
...
RESTARTER="$HOME/.PyCharmEdu20/restart/restarter.sh"
if [ -x "$RESTARTER" ]; then
  "$RESTARTER"
  "$RM" -f "$RESTARTER"
fi
